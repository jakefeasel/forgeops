...
// read the client details from the environment
def oauthClientDetails = (new JsonSlurper()).parseText(System.getenv("OAUTH_CLIENT_DETAILS"))
...
def oauth2Handler = OAuth2AuthHandler.create(authProvider, "http://localhost:8080")
...
router.route("/protected")
    ...
    .handler({ routingContext ->
        // At this point we are logged in using the AM provider. The access and id tokens are saved in the session.
        def user = routingContext.user()

        // If we want to use the details from the id_token to make local authZ decisions, we can get them with user.idToken()
        user.setTrustJWT(true)
        def id_token = user.idToken()

        // Putting the prettily-encoded token into the context so it can be shown in the template, for demo purposes
        routingContext.put("idTokenDetails", (new JsonObject(id_token)).encodePrettily())

        // We can use the access_token associated with the user to make requests to any resource server endpoint which
        // is expecting tokens from AM. For example, these IDM endpoints:
